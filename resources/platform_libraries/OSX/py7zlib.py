...
class StreamsInfo(Base):
    ...
    def __init__(self, file):
        ...
        if id == PROPERTY_SUBSTREAMS_INFO:
            self.substreamsinfo = SubstreamsInfo(file, self.unpackinfo.numfolders,
                                                 self.unpackinfo.folders)
            id = file.read(1)
...
class ArchiveFile(Base):
    ...
    def _read_from_decompressor(self, coder, decompressor, input, level, num_coders,
                                can_partial_decompress=True, with_cache=False):
        size = self._uncompressed[level]
        data = ''
        idx = 0
        cnt = 0
